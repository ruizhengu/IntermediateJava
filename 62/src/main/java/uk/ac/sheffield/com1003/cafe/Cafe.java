...
public class Cafe {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        if (indexNextOrderToPlace > 100) {
            Recipe recipeObjName = findRecipe(recipeName);
            if (recipeObjName == null) {
                throw new RecipeNotFoundException();
            } else {
                double recipeObjPrice = recipeObjName.getPrice();
                if (recipeObjPrice == amountPaid) {
                    orders[indexNextOrderToPlace] = new Order(recipeObjName, customerName, amountPaid);
                    return true;
                } else {
                    return false;
                }
