...
public class Cafe {
    ...
    public void printPendingOrders() {
        ...
        for (int i = indexNextOrderToServe; i < indexNextOrderToPlace; i++) {
            pendingOrders.append(orders[i].toString()).append("\n");
        }
        // Remove the trailing newline character
        System.out.println(pendingOrders.substring(0, pendingOrders.length()-1));
    }

    /**
     * Returns a string of all the recipes on the menu in the format:
     * <Recipe name> - <Recipe price> (one line per recipe)
     */
    private String stringMenuItems() {
        StringBuilder recipes = new StringBuilder();
        for (Recipe recipe: getMenu()) {
            recipes.append(recipe.getName()).append(" - ").append(recipe.getPrice()).append("\n");
        }
