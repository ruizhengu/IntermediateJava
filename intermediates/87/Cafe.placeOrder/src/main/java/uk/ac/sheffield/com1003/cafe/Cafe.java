...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        // Initialize flag variable and index
        boolean recipeFound = false;
        int recipeIndex = 0;

        // Loop through the menu to find the recipe with the specified name
        ...
        // If the recipe is found, remove it from the menu
        if (recipeFound) {
            Recipe[] newMenu = new Recipe[nRecipes-1];
            int j = 0;
            for (int i = 0; i < nRecipes; i++) {
                if (i != recipeIndex) {
                    newMenu[j] = menu[i];
            ...
            // Update the menu and recipe count
            nRecipes--;
            menu = newMenu;
        } else {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        //if the cafe cannot take any more orders and is out of capacity
        if (indexNextOrderToPlace >= orders.length) {
            throw new CafeOutOfCapacityException();
        }
        //if the recipe name does not exist in the menu
