...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        for (int i = 0; i < nRecipes; i++) {
            if (menu[i] != null && recipeName.equals(menu[i].getName())) {
                //removing recipe from menu by assigning as null
                menu[i] = null;
                recipeFound= true;
                //exiting the loop once we find the recipe
                break;
            }
        }
        //checking if the recipe is found in the menu
        if (!recipeFound){
            throw new RecipeNotFoundException();
    }
        // decrementing the number of recipes that are in the menu
        nRecipes--;
    }
    ...
    CafeOutOfCapacityException{
        ...
        // Checking if the cafe has reached its capacity for orders
        ...
        if (amountPaid >= price) {
            ...
            // Adding the new order to the orders array and incrementing the index
    ...
    public Order serveOrder() {
        // Checking if there are any orders left to serve
        if (indexNextOrderToServe >= indexNextOrderToPlace) {
            return null; // No orders to serve
        }

        // Serve the next order in line
