...
public class Cafe {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException
    {
        if(indexNextOrderToPlace >= orders.length) { throw new CafeOutOfCapacityException(); }
            ...
            for (int i = 0; i < this.orders.length; i++) {
                if (this.menu[i].getName() == recipeName && amountPaid >= this.menu[i].getPrice()) {
                    orders[indexNextOrderToPlace] = new Order(this.menu[i], customerName, amountPaid);
                    indexNextOrderToPlace = indexNextOrderToPlace + 1;
                    return true;
                }
                else if (this.menu[i].getName() == recipeName) { return false; }
                else { continue; }
            }
            throw new RecipeNotFoundException();
        }
    ...
    {
        Order tempOrder = orders[indexNextOrderToServe];
        orders[indexNextOrderToServe].serve();
        indexNextOrderToServe = indexNextOrderToServe + 1 ;
