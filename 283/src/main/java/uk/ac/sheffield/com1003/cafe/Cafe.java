...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        // Try find a matching recipe
        for (int i = 0; i < menu.length; ++i) {
            if (menu[i].getName() == recipeName) {
                // Set the recipe to null so it gets overwritten by addRecipe or
                //      ignored by getMenu.
                menu[i] = null;
            }
        }
    ...
    public void printPendingOrders() {
        ...
        // First unserved order -> Most recently placed order
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws CafeOutOfCapacityException, RecipeNotFoundException {
        // Check if there is capacity for the order
        if (indexNextOrderToPlace >= orders.length) throw new CafeOutOfCapacityException();

        // Check if the menu contains the recipe
    ...
    public Order serveOrder() {
        // Check if there is an order to server
        Order order = orders[indexNextOrderToServe];
        if (order == null) return null;

        // Serve the order
        order.serve();
        System.out.println("Served " + order.toString());
