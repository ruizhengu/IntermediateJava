...
public class Cafe {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        /* check whether it is out of capacity or not */
        if (indexNextOrderToPlace >= orders.length) {
            throw new CafeOutOfCapacityException();
        }

        /**
         * An order is generated and indexNextOrderToPlace increments
         * if the recipe exists in the menu and is paid for correctly
         */
        Recipe recipe = findRecipe(recipeName);
        if (recipe == null) {
            throw new RecipeNotFoundException();
        } else if (recipe.getPrice() > amountPaid) {
            return false;
        } else {
