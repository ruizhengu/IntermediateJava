...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException{
        ...
        //Check if the recipe is in the menu
        Recipe recipeFound = findRecipe(recipeName);
        int i = 0;
        if (recipeFound != null){
            while (i < menu.length) {
    ...
    public void printPendingOrders() {
        ...
        for (int k=indexNextOrderToServe; k <indexNextOrderToPlace; k++){
            //Making sure order is not null
            if (orders[k]!=null){
                System.out.println(orders[k].toString());
            }
        }
    }
    ...
    public void printMenu() {
        ...
        System.out.println("==========");
        Recipe[] food = this.getMenu();
        for (int i=0; i<food.length; i++){
            // Making sure the null at the end of array is not printed
            // (when a recipe is removed)
            if (food[i]!=null){
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        int max = this.orders.length;
        if (indexNextOrderToPlace<max) {
    ...
    public Order serveOrder() {
        //checking if the there is any order is yet to be served
        if (indexNextOrderToPlace>indexNextOrderToServe){
            this.orders[indexNextOrderToServe].serve();
            indexNextOrderToServe++;
            return this.orders[indexNextOrderToServe];
        }
        return null;
    }
}
