...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException{
        //use findRecipe method to find the recipe object with given name in the menu array
        //remove the recipe if it is in the menu
        for (int i = 0; i < getMenu().length; i++) {
            if (findRecipe(recipeName) == menu[i]) {
               menu[i] = null;
            }
        }
        //if the provided recipe name does not exist, throw the exception
    ...
    public void printPendingOrders() {
        ...
        //the Pending Orders are the orders that already be placed but have not been served
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        if (findRecipe(recipeName) != null && findRecipe(recipeName).getPrice() <= amountPaid) {
            if (indexNextOrderToPlace <= orders.length) {
                if (findRecipe(recipeName).isReady() == true) {
                    //if all the conditions are fulfilled, place the order
                    ...
                    if (orders[indexNextOrderToPlace] == null) {
                        ...
                        //indexNextOrderToPlace is initialised as 0
    ...
    public Order serveOrder() {
        if (orders[indexNextOrderToServe] != null && indexNextOrderToServe < orders.length) {
            //if all the conditions are fulfilled, serve the order
            orders[indexNextOrderToServe].serve();
            //indexNextOrderToServe is initialised as 0
