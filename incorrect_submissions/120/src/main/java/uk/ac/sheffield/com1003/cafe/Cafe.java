...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        for (int i = 0; i < menu.length; i++) {
            if (menu[i].getName() == recipeName) {
                // We are essentially removing the pointer, the garbage collector will clean up
                // memory losses. It is also fine to set to null as addRecipe iterates over the
                // whole menu finding a suitable location to add new recipes
                menu[i] = null;
                foundRecipe = true;
            }
        }
    ...
    public void printPendingOrders() {
        ...
        int ordersRemaining = indexNextOrderToPlace - indexNextOrderToServe;
    ...
    public void printMenu() {
        // Gets menu items and prints out in format
        Recipe[] myMenu = this.getMenu();
        ...
        for (int i = 0; i < myMenu.length; i++) {
            System.out.println(myMenu[i].getName() + " - " + myMenu[i].getPrice());
        }
        System.out.println("==========");
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        Recipe recipeSearch = this.findRecipe(recipeName);
        if (recipeSearch == null) {
            throw new RecipeNotFoundException();
        }
        if (amountPaid < recipeSearch.getPrice()) {
            return false;
        }

        // Adds new order to array
        orders[indexNextOrderToPlace] = new Order(recipeSearch, customerName, amountPaid);
    ...
    public Order serveOrder() {
        ...
        Order upcomingOrder = orders[indexNextOrderToServe];
