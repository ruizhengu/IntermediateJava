...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        //stays false to continue running the code
        boolean found = false;
        //run a loop through the entire menu to find the recipe
        for (int i = 0; i < menu.length; i++) {
           if (menu[i] != null && menu[i].getName().equals(recipeName)) {
            ...
            //setting the memory location occupied by the removed recipe to null
            menu[menu.length-1] = null;
            //stoping the loop once the recipe is found
            break;
          }
        }
    ...
    public Order serveOrder() {
      //Comparing to see if there are any pending orders to be served
      if (indexNextOrderToServe < indexNextOrderToPlace) {
        //if yes then the order is served
      Order orderToServe = orders[indexNextOrderToServe];
      orderToServe.serve();
      indexNextOrderToServe++;
      return orderToServe;
      } else {
        //if no order to be served then it returns null
        return null;
      }
    }
}
