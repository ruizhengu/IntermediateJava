...
public class Cafe {
    ...
    public boolean addRecipe(Recipe newRecipe) {
        ...
        while (i < menu.length && !added) {
            if (menu[i] == null) {
                menu[i] = newRecipe;
                nRecipes++; //Increase number of valid recipes in the menu
                added = true;
            } else
                i++; //Searching through the menu for spaces
        }
        return added;
    }
        ...
        if(findRecipe(recipeName)==null){
            ...
            for(int i=0; i<getMenu().length; i++){
                if(menu[i].getName().equals(recipeName)){
                    menu[i]=null;
                    nRecipes--; //Decrease the number of valid recipe
                }
            }
        }
    }
    ...
    /*
     * Overloading method for taking extra parameter "special request"
     */
    public boolean placeOrder(String recipeName, String customerName, double amountPaid, String specialRequest)
        ...
        if(findRecipe(recipeName)!=null&&amountPaid>=findRecipe(recipeName).getPrice()){
            Order order = new Order(findRecipe(recipeName), customerName, amountPaid, specialRequest);
    ...
    public Order serveOrder() {
        if(indexNextOrderToPlace==indexNextOrderToServe){
            ...
            return orders[indexNextOrderToServe-1];//Return the updated order with time served
        }
    }
}
