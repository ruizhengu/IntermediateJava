...
public class Cafe {
    ...
    RecipeNotFoundException {
      /*if resteraunt is full back out instantly*/
        if (indexNextOrderToPlace >= orders.length) {
          throw new CafeOutOfCapacityException();
        }
        else {
          /*checks for recipe, and if its not found, throw an error*/
          Recipe orderRec = findRecipe(recipeName);
          if (orderRec != null) {
              orders[indexNextOrderToPlace] = new Order(orderRec,customerName,amountPaid);
    ...
    public Order serveOrder() {
       /*if the next order to serve is not less than to place, it can be presumed that all orders have been served*/
