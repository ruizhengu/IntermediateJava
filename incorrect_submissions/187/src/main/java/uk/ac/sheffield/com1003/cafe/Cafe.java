...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        //Task 7
        if (findRecipe(recipeName).getName().equals(recipeName)) {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        if (indexNextOrderToPlace >= orders.length) { //Order FULL
        ...
        if (findRecipe(recipeName).getName().equals(recipeName)) {
            if (amountPaid>=findRecipe(recipeName).getPrice()) {
                ...
                orders[indexNextOrderToPlace] = newOrder; //place order onto array
                indexNextOrderToPlace++; //Increment indexNextOrderToPlace for next order
                return true;
            } else {
                return false; //insufficient fund
            }
        } else { //RECIPE NAME NOT MATCH
            throw new RecipeNotFoundException();
        }
    }
    ...
    public Order serveOrder() {
        if (orders[indexNextOrderToServe]==null) { //if null, no order to serve
            return null;
        } else {
