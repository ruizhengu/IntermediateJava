...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        // find recipe using findRecipe()
        Recipe recipe = findRecipe(recipeName);

        // since above function returns null if not found,
        // throw exception if null
        if (recipe == null)
            throw new RecipeNotFoundException();

        // finds recipe in menu and change it to null (default)
    ...
    public void printMenu() {
        ...
        // first 4 consistent lines
        String menu = "==========\n" + greeting() + "\nMenu\n==========\n";

        // get array of recipes available
        Recipe[] actual_menu = getMenu();

        // 1 line for each recipe
        for (Recipe i : actual_menu)
            menu += i.getName() + " - " + String.format("%.1f", i.getPrice()) + "\n";

        // last 2 lines and print string
        menu += "==========\nEnjoy!";
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        // if last entry in orders is not null then orders is full
        boolean order_full = orders[orders.length - 1] == null;

        // throw exception if order list is full
        if (order_full)
            throw new CafeOutOfCapacityException();

        // find recipe and throw exception if it doesn't exist
        Recipe recipe = findRecipe(recipeName);
        if (recipe == null)
            throw new RecipeNotFoundException();

        // check if amount paid is enough
        boolean enough = amountPaid >= recipe.getPrice();

        // if amount paid is enough, place order
        if (enough) {
    ...
    public Order serveOrder() {
        ...
        // if toPlace and toServe is the same index that means no orders are pending
        if (indexNextOrderToPlace == indexNextOrderToServe)
            return null;
        // serve order and increment counter
        else {
            orders[indexNextOrderToServe].serve();
            indexNextOrderToServe++;
        }
        
        // return the order that's just served
