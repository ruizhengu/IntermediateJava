...
public class Cafe {
    ...
    public String greeting() {
        return "Welcome to "+this.getName().toString();
    }
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
    	...
    	// Search menu array for the recipe passed as an argument and set it to null
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
    	...
    	try {
    		//check that the cafe still has capacity
    		if (indexNextOrderToPlace<=orders.length) {
    			
    	   		//check if the recipe exists
            	if (currentRecipe!= null) {
            		
            		//check that the customer has paid enough and that the recipe is ready before creating the new order
            		if (currentRecipe.isReady()&& amountPaid >= currentRecipe.getPrice()) {
    ...
    public Order serveOrder() {
    	//Check that there are orders to serve, and that there have not been too many orders served
    	if ((orders.length > 0) && (indexNextOrderToServe<orders.length)){
