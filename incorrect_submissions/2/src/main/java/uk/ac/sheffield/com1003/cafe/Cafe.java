...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        //if the recipe isn't found then throw the exceptiob
        if (!recipeFound) {
...
//        System.out.println("Menu to come soon. For guidance, see Javadoc above and TestCafe.testPrintMenu()");
        ...
        for (int i = 0; i < this.getMenu().length; i++) {
            // makes a new array using getMenu and finds the name and price
            if (this.getMenu()[i] != null) {
                System.out.println(this.getMenu()[i].getName() + " - "
                        + this.getMenu()[i].getPrice());
            }
        }
        System.out.println("==========");
    ...
    public Order serveOrder() {
        //if they're equal and below the capacity by 1 index
        if (indexNextOrderToServe < orders.length - 1 && indexNextOrderToServe < indexNextOrderToPlace) {
            // increment indexNextOrderToServe and serve the order
