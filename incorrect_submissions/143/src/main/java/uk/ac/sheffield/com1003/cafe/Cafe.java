...
public class Cafe {
     ...
     * loops through menu to find the recipe name.
     * @param recipeName Name of the recipe to be removed
     * @throws RecipeNotFoundException If a recipe with the given name is not found
     */
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        boolean recipeFound = false;
        int i = 0;
        while (i < menu.length && !recipeFound) {
            if (menu[i] != null && menu[i].getName().equals(recipeName)) {
                // shift all elements to the left to remove the element at index i
                for (int j = i; j < menu.length - 1; j++) {
                    menu[j] = menu[j + 1];
                }
                menu[menu.length - 1] = null; // set the last element to null
                recipeFound = true;
            }
            i++;
        }
    ...
    public void printMenu() {
        ...
        System.out.println("==========");
        //loops through the menu array to print each recipe
        for (int i = 0; i < menu.length; i++) {
            if (menu[i] != null) {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        Order order = new Order(requestedRecipe, customerName, amountPaid);
    ...
    public Order serveOrder() {
        ...
        if (indexNextOrderToServe < indexNextOrderToPlace) {
            order = orders[indexNextOrderToServe];
            order.serve();
