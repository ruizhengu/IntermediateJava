...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        while (i < menu.length && !removed) {
            if (menu[i] != null && menu[i].getName().equalsIgnoreCase(recipeName)) {
                menu[i] = null;
                nRecipes--;
                removed = true;
            } else i++;
...
//        System.out.println("Menu to come soon. For guidance, see Javadoc above and TestCafe.testPrintMenu()");
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid, String specialRequest) throws CafeOutOfCapacityException, RecipeNotFoundException {
        ...
        boolean found = false;
        while (i < menu.length && !found) {
            if (menu[i].getName().equalsIgnoreCase(recipeName)) {
                found = true;
                if (menu[i].getPrice() <= amountPaid) {
                    orders[indexNextOrderToPlace] = new Order(menu[i], customerName, amountPaid, specialRequest);
                    indexNextOrderToPlace++;
                    return true;
                } else {
                    return false;
    ...
    private Recipe findRecipe(String recipeName) {
        int i = 0;
        while (i < menu.length) {
            if (menu[i] != null && menu[i].getName().equals(recipeName)) return menu[i];
            i++;
        }
        return null;
    }
