...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        while (i < menu.length) {
            if (menu[i] != null && menu[i].getName().equals(recipeName)) {
                // set current position of menu to null, thus to delete the recipe.
                menu[i] = null;
                return;
            }
            i++;
        }
        // if no recipe named recipeName found, throw the Exception.
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        this.orders[this.indexNextOrderToPlace] = new Order(recipe, customerName, amountPaid);
    ...
    public Order serveOrder() {
        Order order = this.orders[this.indexNextOrderToServe];
        if (order != null) {
            order.serve();
            this.indexNextOrderToServe += 1;
            return order;
        } else {
            return null;
        }
