...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        for (int i=0; i<menu.length; i++) {
            if (menu[i].getName() == recipeName) {
                menu[i] = null;
                //Shifting the menu to remove null entry.
                for (int j=0; j<menu.length; j++) {
                    if (menu[j] == null) {
                        for (int k=j; k<menu.length-1; k++) {
                            menu[k] = menu[k+1];
                        }
                    }
                }
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        Recipe item = new Recipe("Drink", 0);
        if (findRecipe(recipeName) != null) {
            item = findRecipe(recipeName);
        }
        else {
            throw new RecipeNotFoundException();
        }
    ...
    public Order serveOrder() {
        ...
        int order = indexNextOrderToServe;
        indexNextOrderToServe++;
        orders[order].serve();
        orders[order].printReceipt();
        return orders[order];
    }
}
