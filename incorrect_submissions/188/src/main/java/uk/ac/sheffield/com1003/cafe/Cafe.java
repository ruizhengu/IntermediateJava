...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
    	for(int i=0;i<menu.length;i++) {
    		//skip null recipes (avoid getName() nullPointer error)
    	...
    	//if the loop exits without returning early then the recipe was not in the menu.
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
    	for(int i=0; i<getMenu().length; i++) {
    		if(getMenu()[i] == null) continue;
    		if(getMenu()[i].getName() == recipeName) {
    			if(amountPaid >= getMenu()[i].getPrice()) {
    				if(indexNextOrderToPlace>=orders.length) throw new CafeOutOfCapacityException();
    				else {
    					orders[indexNextOrderToPlace] = new Order(getMenu()[i],customerName,amountPaid);
