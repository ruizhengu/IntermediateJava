...
public class Cafe {
    ...
    public void printMenu() {
        ...
        //loops through all of the available objects on the menu and only displays their name and price as long as an item exists in the index position
        for(int i = 0; i < nRecipes; i++)
        {
            if(menu[i] != null)
            {
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
       ...
       if(amountPaid < recipe.getPrice())
       {
        ...
        //if the system is trying to add an order when the cafe is already at capacity it will throw an exception
    ...
    public Order serveOrder() {
        //will return null if there are no orders
        if(indexNextOrderToPlace == 0 || indexNextOrderToPlace == indexNextOrderToServe)
        {
            return null;
        }
