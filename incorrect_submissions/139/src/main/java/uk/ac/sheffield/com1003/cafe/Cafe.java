...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException{
        int matchIndex = -1; //stores the index where the match is
        for(int i=0;i<menu.length;i++)
            if (recipeName== menu[i].getName()){
                matchIndex = i;
                break;
            }
        if(matchIndex == -1){
    ...
    public void printMenu() {
        ...
        System.out.println("==========");
        for(int i=0;i<menu.length;i++) {
        	if(menu[i]!=null) {System.out.println(menu[i].getName()+" - "+menu[i].getPrice());}
        };
        System.out.println("==========");
    ...
    CafeOutOfCapacityException{
    	int i = 0;
    	while (i<menu.length) {
    		if (recipeName == menu[i].getName() && menu[i].getName() != null ) break;
    		i += 1;
    	}
    	if (i == menu.length) { //if i is equal to this then the recipe isn't on the menu.
    		throw new RecipeNotFoundException();
    	}
    ...
    public Order serveOrder() {
    	//updates next order variable, sets serve to true in object and returns
