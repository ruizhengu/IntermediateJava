...
public class Cafe {
     ...
     * creates a temporary menu which is the original menu
     * but without the recipe that you wanted to remove
     * it then makes the temp recipe = to the menu so that
     * its updated
     * 
     ...
     * it checks exceptions first and throws exceptions if conditions are met
     * then it checks the price to see if it is sufficent
     * 
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid)
        ...
        if (checkRecipe == null) {
            ...
            Order orderToDo = new Order(checkRecipe, customerName, amountPaid);
            orders[indexNextOrderToPlace] = orderToDo;
            indexNextOrderToPlace = indexNextOrderToPlace + 1;
            return true;
        }
        return false;
