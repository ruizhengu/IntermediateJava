...
public class Cafe {
  ...
  public void printMenu() {
    ...
    System.out.println(SEPERATOR);
    for (Recipe r : this.getMenu()) {
      // Print if recipe is not null
      // Recipe can be null if it has been removed.
      if(r != null) {
  ...
  public boolean placeOrder(String recipeName, String customerName, double amountPaid)
    ...
    // Return false if there are recipes
    if (!(nRecipes > 0)) return false;
    // Find recipe
    ...
    if (amountPaid < menuItem.getPrice()) return false;

    Order order = new Order(menuItem, customerName, amountPaid);
  ...
  public Order serveOrder() {
    // return null if orderToServe >= orderToPlace
    // since orderToPlace increments to out of bounds (for the orders array)
