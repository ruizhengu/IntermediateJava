...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        for (int i = 0; i < nRecipes; i++) {
            // If the menu item matches the given name,
            if (menu[i].getName() == recipeName) {
                recipeFound = true;
                // Delete it,
                menu[i] = null;
                // And move all elements after it back
    ...
    public void printPendingOrders() {
        ...
        // Iterating over all pending orders in the orders array,
        for (int i = indexNextOrderToServe; i < orders.length; i++) {
            // If the last order has been reached, quit iterating
            if (orders[i] == null) break;
            // Otherwise, display the order
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        // Search for the specified recipe
        ...
        // If there is not enough room for a new order,
        if (indexNextOrderToPlace >= orders.length) {
            // Throw an exception
            throw new CafeOutOfCapacityException();
        }
        // Add the new order into the orders array
        orders[indexNextOrderToPlace] = new Order(desiredRecipe, customerName, amountPaid);
    ...
    public Order serveOrder() {
        // If there is no order to serve, return null
        if (orders[indexNextOrderToServe] == null) return null;
        // Otherwise, serve the order
        orders[indexNextOrderToServe].serve();
        // And increment indexNextOrderToServe
        indexNextOrderToServe++;
        // Then return the newly served order
