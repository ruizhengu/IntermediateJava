...
private int orderCapacity;
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        Recipe[] z = new Recipe[menu.length - 1];
        int j = 0;
        for (int i = 0; i < menu.length; i++) {
            if (i != indexToRemove) {
                z[j++] = menu[i];
    ...
    public Order serveOrder() throws CafeOutOfCapacityException {
        /*It is the same concept of the place order if the index next order to serve it will throw exception*/
        if (indexNextOrderToServe == 100) {
            throw new CafeOutOfCapacityException();

        }/*if the index next order to place is increment by one , the index next order to serve will also increment by one*/
        if ( indexNextOrderToPlace == indexNextOrderToPlace + 1){
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        for (Recipe i : menu) {
            if (recipeName.equals(i.getName())) {
                ...
                recipe = i;
                break; // exit loop since we found the recipe
            }
        }

        if (recipe == null) {
            throw new RecipeNotFoundException();
        }else if (orderCapacity > 100) {
            throw new CafeOutOfCapacityException();
        } else {
            try {
