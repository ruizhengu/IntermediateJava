...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        // check for recipe, if it is there, remove it
    ...
    public void printMenu() {
        ...
        System.out.println("==========");
        Recipe[] printedMenu = getMenu();
        for (int i = 0; i < printedMenu.length; i++) {
            System.out.println(String.format("%s - %s", printedMenu[i].getName(), printedMenu[i].getPrice()));
        }
        System.out.println("==========");
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        if (recipe.getPrice() <= amountPaid) {
            ...
            this.orders[indexNextOrderToPlace] = order;
    ...
    public Order serveOrder() {
        if (indexNextOrderToServe >= this.orders.length) {
            return null;
        }
        Order order = this.orders[indexNextOrderToServe];
        if (order == null) {
            return null;
        }
        order.serve();
