 ...
 *The Cafe class represents a cafe object with a name, a menu consisting of recipes, and
 *an array of orders.
 */
...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        menu = newMenu.toArray(new Recipe[newMenu.size()]);
    }
...
//    }
    ...
    public void printMenu() {
    	...
    	System.out.println("==========");
    	Recipe[] menus = getMenu();
    	for (Recipe recipe: menus) {
    		if (recipe != null) {
	...
	public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
    	Recipe recipeToBeOrdered = findRecipe(recipeName);
    	if (recipeToBeOrdered!=null && recipeToBeOrdered.getPrice() <= amountPaid) {
    		Order newOrder = new Order(recipeToBeOrdered, customerName, amountPaid);
            orders[indexNextOrderToPlace] = newOrder;
            indexNextOrderToPlace++;
    		return true;
    	}
    	
    	if (recipeToBeOrdered == null) {
    		throw new RecipeNotFoundException();
    	}
    ...
    public Order serveOrder() {
    	if (orders.length > 0) {
    		if (indexNextOrderToServe < indexNextOrderToPlace) {
		        ...
		        orderToServe.serve(); //changing the served time from null to now
		        indexNextOrderToServe++;
		        return orderToServe;
		    } else {
		        return null; // no orders left to serve
		    }
    	}
		return null;
