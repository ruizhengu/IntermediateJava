...
public class Cafe {
    ...
    public void printPendingOrders() {
        ...
        // Print from serve index to (order index - 1)
        for (int i = 0; i + indexNextOrderToServe <= indexNextOrderToPlace - 1; i++) {
            System.out.println(orders[indexNextOrderToServe + i].toString());
        }
    }
    ...
    public void printMenu() {
        String recipeMenu = "";
        // Format Recipes to string
        for (Recipe recipe : getMenu()) {
            // No recipes in menu check as getMenu may return a null array
            if (recipe != null) {
                recipeMenu += recipe.getName() + " - " + recipe.getPrice() + "\n";
            }
        }
        String menuString = 
        "==========\n" +
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        // Exception throws
        if (indexNextOrderToPlace >= orders.length) { throw new CafeOutOfCapacityException(); } // Exceeded order capacity
    ...
    public Order serveOrder() throws CafeOutOfCapacityException {
        if (indexNextOrderToServe >= orders.length) { throw new CafeOutOfCapacityException(); }
