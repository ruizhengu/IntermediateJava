...
public class Cafe {
    ...
    public void removeRecipe(String recipeName) throws RecipeNotFoundException {
        ...
        //Finds the index of the recipe to be removed
        for (int i = 0; i < menu.length; i++){
            if (menu[i] != null && recipeName == menu[i].getName()) {
                recipeIndex = i;
                recipeFound = true;
            }
        }

        //Lowers the index of each recipe after the one to be removed
        if (recipeFound) {
            while (menu[recipeIndex] != null){
                menu[recipeIndex] = menu[recipeIndex + 1];
                recipeIndex++;
            }
        } else
