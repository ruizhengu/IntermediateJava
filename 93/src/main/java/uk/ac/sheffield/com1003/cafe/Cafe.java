...
public class Cafe {
        ...
        for (int i=0; i<menu.length; i++){
            if (menu[i] != null){
                if (recipeName.equals(menu[i].getName())){
                    foundRecipe = true;
                    //shift menu to the left to remove recipe
                    for (int j=i; j<menu.length-1; j++) {
                        menu[j] = menu[j+1];
                    }
                    //replace final value in menu with null
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid) throws RecipeNotFoundException, CafeOutOfCapacityException {
        //check if there is space to order
        if (indexNextOrderToPlace >= orders.length){
            throw new CafeOutOfCapacityException();
        }

        //Check if recipe is in menu
        for (int i=0; i<menu.length; i++){
    ...
    public boolean placeOrder(String recipeName, String customerName, double amountPaid, String specialRequest) throws RecipeNotFoundException, CafeOutOfCapacityException {
        ...
        for (int i=0; i<menu.length; i++){
            if (menu[i].getName().equals(recipeName)){
                if (menu[i].getPrice()<= amountPaid){
                    //create and add new order to orders list
                    Order order = new Order(menu[i], customerName, amountPaid, specialRequest);
                    orders[indexNextOrderToPlace] = order;
                    this.indexNextOrderToPlace++;
                    return true;
                }
